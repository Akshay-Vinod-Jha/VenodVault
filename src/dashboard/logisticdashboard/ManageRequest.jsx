...
const LManageRequest = () => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      collection(db, `logistic/${logisticId}/requests`),
      (snapshot) => {
  ...
  const updateStatus = async (requestId, newStatus) => {
    try {
      const reqRef = doc(db, `logistic/${logisticId}/requests`, requestId);
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6 text-white">
        Manage Logistic Requests
      </h2>

      {requests.length === 0 ? (
        <p className="text-gray-400">No logistic requests available.</p>
      ) : (
            ...
            >
              <div className="text-white">
                ...
                <p className="text-sm mt-1 text-gray-300">
                  Pickup:{" "}
                  <span className="text-white">{req.pickupLocation}</span> →
                  Drop: <span className="text-white">{req.dropLocation}</span>
                </p>
